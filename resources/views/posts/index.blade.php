        ...
        @foreach ($posts as $post)
            <div class="col-12 col-md-4 mt-3">
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        <h5 class="card-title"> <a href="{{ route('posts.show', $post->slug) }}">{{ $post->title }}</a>
                        </h5>
                        <p class="card-text">{{ $post->content }}</p>
                        <p class="card-text"> user name : <a href="{{ route('user.profile',$post->user->name) }}">{{ $post->user->name }}</a></p>
